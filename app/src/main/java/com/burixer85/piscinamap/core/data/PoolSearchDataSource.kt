...
@Singleton
class PoolSearchDataSource @Inject constructor(
        ...
        return try {
            ...
            val pools = response.results
                .filter { place ->
                    ...
                    !GENERIC_NAMES.any { name.trim() == it } &&
                        EXCLUDE_PATTERNS.none { name.contains(it) }
                }
                .map { place ->
            ...
            val ttlThreshold = System.currentTimeMillis() - CACHE_TTL_MS
    ...
    companion object {
        ...
        private val GENERIC_NAMES = listOf("swimming pool", "piscina", "pool")
        private val EXCLUDE_PATTERNS = listOf(
            "piscinas", "piscina s", "piscinas triana", "acuaeuropa",
            "piscinas sevilla", "piscina sevilla", "piscina madrid",
            "tienda", "tienda de", "ventas", "venta de",
            "s.l.", " s.l", "sl", "sa", "s.a.", "slne"
        )
    }
}
